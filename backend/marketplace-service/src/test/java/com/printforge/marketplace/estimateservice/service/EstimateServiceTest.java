package com.printforge.marketplace.estimateservice.service;

import com.printforge.marketplace.entity.Role;
...
import com.printforge.marketplace.estimateservice.repository.EstimateRepository;
...
/**
 * Pure unit test, no Spring context or DB. Ported from order-service's
 * EstimateServiceTest (itself ported from the monolith) — proves the size
 * used in the cost calculation comes from the actual stored file, not a
 * number the client made up, and that bad quality/materialType/fileId are
 * rejected instead of silently falling back to a default.
 *
 * materialRepository is mocked with fixtures matching the exact cost/
 * density/baseMinutesPerGram values order-service's MaterialSeeder seeds
 * in production (this service shares the same `materials` table but does
 * not seed it itself) — every test's hardcoded expected number below is
 * unchanged from before the materials-table migration.
 *
 * Run with: ./mvnw -pl marketplace-service test -Dtest=EstimateServiceTest
 */
