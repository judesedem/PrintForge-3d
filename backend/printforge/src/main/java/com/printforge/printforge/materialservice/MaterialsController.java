 ...
 * .Material) — the same table EstimateService reads cost_per_gram/
 * baseMinutesPerGram/density from for its cost calculation, so this
 * listing can never drift out of sync with what a customer is actually
 * charged the way it previously did (PETG/CARBON_FIBER had two different
 * hardcoded cost_per_gram values before this migration — see
 * config/MaterialSeeder.java's javadoc for the full history). Admin edits
 * via PATCH /api/admin/materials/{name} (AdminController) show up here
 * immediately, same table, no caching.
 *
 ...
 * as an opaque string key (Frontend/src/api/materials.ts's Material.id),
 * never parses or compares it, so this is not a breaking change.
 */
@RestController
...
public class MaterialsController {
    ...
    // (Frontend/src/api/materials.ts's MaterialApiResponse) stays identical.
