 ...
 * PrintQueueServiceTest — covers job creation ownership checks, status
 * validation, and printer-status cascading (assigning a printer flips it
 * to BUSY, completing/failing a job frees it).
 *
 * Run with: ./mvnw -pl order-service test -Dtest=PrintQueueServiceTest
 */
class PrintQueueServiceTest {
    ...
    /**
     * Regression test: the assignment check only rejected printers whose
     * status was the literal string "BUSY", which let OFFLINE and
     * MAINTENANCE printers slip through and get silently flipped to BUSY.
     * Both must be rejected the same way BUSY is.
     */
    @ParameterizedTest
    ...
    void readyViaTransitionStillUsesTheGenericSuccessType() {
        // READY isn't one of the specific categories notification type
        // granularity introduced — it must keep using the generic
        // "success" type, unchanged.
        PrintJob existing = new PrintJob();
        existing.setId(5L);
        existing.setUserId(42L);
        existing.setStatus("PRINTING");
